	...
	<div id="wrapper">
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container">
					<h1 align="center">BILL LIST</h1>
					<a href="addBill.jsp" class="btn btn-success" style="float: right">ADD BILL</a><br> <br>
					<table id="customers">
						<tr>
							<th>BILL ID</th>
							<th>BILL NAME</th>
						...
						<c:forEach items="${bills}" var="bill" varStatus="bills">
							<tr>
								...
								<td><a
									href="BillController?action=viewBill&bid=<c:out value="${bill.bid}" />"
									class="btn btn-warning">VIEW</a></td>
								<td><a
									href="BillController?action=updateBill&bid=<c:out value="${bill.bid}" />"
									class="btn btn-primary">UPDATE</a></td>
								<td><input type="hidden" id="bid-${bills.index}"
									value="<c:out value="${bill.bid}"/>">
								<button class="btn btn-danger"
										onclick="confirmation('${bills.index}')">DELETE</button></td>
						</c:forEach>
					</table>
		...
		<script src="js/sb-admin-2.min.js"></script>
				...
				<script>
					function confirmation(index) {
						var bid = $("#bid-" + index).val();

						console.log(bid);
						var r = confirm("Are you sure you want to delete?");
						if (r == true) {
							location.href = 'BillController?action=deleteBill&bid='
									+ bid;
